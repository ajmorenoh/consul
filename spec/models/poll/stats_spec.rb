...
describe Poll::Stats do
  ...
  describe "#participants_by_geozone" do
    ...
    it "doesn't calculate population percentage because geozones don't have population" do
      midgar = create(:geozone, name: "Midgar")

      create(:poll_voter, poll: poll, user: create(:user, :level_two, geozone: midgar))

      expect(stats.participants_by_geozone["Midgar"][:population_percentage]).to eq 0.0
    end
