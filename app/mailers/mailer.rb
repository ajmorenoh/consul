class Mailer < ApplicationMailer
  ...
  def comment(comment)
    ...
    unless Rails.env.production?
      with_user(@commentable.author) do
        subject = g('mailers.comment.subject', commentable: t("activerecord.models.#{@commentable.class.name.underscore}", count: 1).downcase)
  ...
  def reply(reply)
    ...
    unless Rails.env.production?
      with_user(@recipient) do
        mail(to: @email_to, subject: g('mailers.reply.subject')) if @commentable.present? && @recipient.present?
      end
    end
  end
  ...
  def email_verification(user, recipient, token, document_type, document_number)
    ...
    with_user(user) do
      mail(to: @email_to, subject: g('mailers.email_verification.subject'))
    end
  end
  ...
  def unfeasible_spending_proposal(spending_proposal)
    ...
    with_user(@author) do
      mail(to: @email_to, subject: g('mailers.unfeasible_spending_proposal.subject', code: @spending_proposal.code))
    end
  end
  ...
  def direct_message_for_receiver(direct_message)
    ...
    with_user(@receiver) do
      mail(to: @email_to, subject: g('mailers.direct_message_for_receiver.subject'))
    end
  end
  ...
  def direct_message_for_sender(direct_message)
    ...
    with_user(@sender) do
      mail(to: @email_to, subject: g('mailers.direct_message_for_sender.subject'))
    end
  end
  ...
  def proposal_notification_digest(user, notifications)
    ...
    with_user(user) do
      mail(to: @email_to, subject: g('mailers.proposal_notification_digest.title', org_name: Setting['org_name']))
    end
  end
  ...
  def user_invite(email)
    ...
    I18n.with_locale(I18n.default_locale) do
      mail(to: @email_to, subject: g('mailers.user_invite.subject', org_name: Setting["org_name"]))
    end
  end
  ...
  def budget_investment_created(investment)
    ...
    with_user(@investment.author) do
      mail(to: @email_to, subject: g('mailers.budget_investment_created.subject'))
    end
  end
  ...
  def budget_investment_unfeasible(investment)
    ...
    with_user(@author) do
      mail(to: @email_to, subject: g('mailers.budget_investment_unfeasible.subject', code: @investment.code))
    end
  end
  ...
  def budget_investment_selected(investment)
    ...
    with_user(@author) do
      mail(to: @email_to, subject: g('mailers.budget_investment_selected.subject', code: @investment.code))
    end
  end
  ...
  def budget_investment_unselected(investment)
    ...
    with_user(@author) do
      mail(to: @email_to, subject: g('mailers.budget_investment_unselected.subject', code: @investment.code))
    end
  end
