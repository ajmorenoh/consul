namespace :i18n_contents do
  desc "Generate the keys and the translations for the I18n contents table"
  task generate: :environment do

    key = I18nContent.create(key: 'budgets.index.section_footer.description')
    key.update(value_es: 'Con los presupuestos participativos la ciudadanía decide a qué proyectos presentados por los vecinos y vecinas de Madrid va destinada una parte del presupuesto municipal.')
    key.update(value_en: 'With the participatory budgets the citizens decide to which projects presented by the neighbors of Madrid is destined a part of the municipal budget.')

    key = I18nContent.create(key: 'budgets.index.section_footer.help_text_1')
    key.update(value_es: "Los presupuestos participativos son unos procesos en los que la ciudadanía decide de forma directa en qué se gasta una parte del presupuesto municipal. Cualquier persona empadronada en Madrid mayor de 16 años puede proponer un proyecto de gasto que se preselecciona en una fase de apoyos ciudadanos.")
    key.update(value_en: "Participatory budgets are processes in which citizens decide directly on what is spent part of the municipal budget. Any registered person in Madrid over 16 years old can propose an investment project that is preselected in a phase of citizen supports.")

    key = I18nContent.create(key: 'budgets.index.section_footer.help_text_2')
    key.update(value_es: "Los proyectos más votados se evalúan y pasan a una votación final en la que se deciden las actuaciones que llevará a cabo el Ayuntamiento de Madrid una vez se aprueben los presupuestos municipales del año próximo.")
    key.update(value_en: "The most voted projects are evaluated and passed to a final vote in which they decide the actions to be carried out by the City Council of Madrid once the municipal budgets of the next year are approved.")
    ...
    key.update(value_es: "¡Adiós! Tu cuenta ha sido cancelada. Esperamos volver a verte pronto. Le informamos que de conformidad con su petición,
    ...
    key.update(value_es: 'Te damos la bienvenida al Portal de Gobierno Abierto del Ayuntamiento de Madrid')
    key.update(value_en: 'Welcome to the Open Government Portal of the Madrid City Council')
    ...
    key.update(value_es: 'Las votaciones ciudadanas del Ayuntamiento de Madrid son un mecanismo de participación por el que la ciudadanía con derecho a voto puede tomar decisiones municipales de forma directa. Pueden participar todas aquellas personas mayores de 16 años que estén empadronadas en Madrid.')
    key.update(value_en: "Madrid City Council's citizens' votes are a participatory mechanism through which citizens with the right to vote can make municipal decisions directly. Anyone over 16 years old who is registered in Madrid can participate.")

    key = I18nContent.create(key: 'polls.index.section_footer.help_text_1')
    key.update(value_es: "Las consultas se convocan de manera periódica, bien porque el Ayuntamiento somete cuestiones de su competencia a decisión directa, bien porque una propuesta ciudadana llega al umbral de apoyos necesarios y pasa a votación.")
    key.update(value_en: "Consultations are held on a regular basis, either because the City Council submits questions of its competence for direct decision, or because a citizens' proposal reaches the threshold of necessary support and goes to the vote.")

    key = I18nContent.create(key: 'polls.index.section_footer.help_text_2')
    key.update(value_es: "En 2017 tuvieron lugar las primeras votaciones ciudadanas de la Ciudad de Madrid. Por un lado, el Ayuntamiento preguntó sobre la reforma de Plaza de España, la movilidad de la Gran Vía y la remodelación de espacios públicos en once distritos de Madrid.")
    key.update(value_en: "In 2017 the first citizens' votes took place in the City of Madrid. On the one hand, the City Council asked about the refurbishment of Plaza de España, the mobility of the Gran Vía and the remodelling of public spaces in eleven districts of Madrid.")
    ...
    key.update(value_es: "Desde el Ayuntamiento de Madrid queremos agradecer tu participación en los <strong>Presupuestos Participativos de la ciudad de Madrid</strong>. Lamentamos informarte de que tu propuesta <strong>'%{title}'</strong> quedará excluida de este proceso participativo por el siguiente motivo:")
    key.update(value_en: "From the Madrid City Council we want to thank you for your participation in the <strong>participatory budgets of the city of Madrid</strong>. We regret to inform you that your proposal <strong>'%{title}'</strong> will be excluded from this participatory process for the following reason:")
    ...
    key.update(value_es: "Si consideras que el proyecto rechazado cumple los requisitos para mantenerlo como proyecto de gasto, podrás comunicarlo, en el plazo de 48 horas, al correo preparticipativos@madrid.es, indicando necesariamente para su tramitación el código %{code} como asunto del correo, correspondiente a tu proyecto.")
    ...
    key.update(value_es: "Desde el Ayuntamiento queremos agradecer tu participación en los <strong>Presupuestos Participativos</strong>. Lamentamos informarte de que tu proyecto <strong>'%{title}'</strong> quedará excluido de este proceso participativo por el siguiente motivo:")
