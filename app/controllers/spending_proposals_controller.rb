class SpendingProposalsController < ApplicationController
  ...
  def results
    ...
    @spending_proposals = daily_cache("sps_geozone_#{params[:geozone_id]}") { Budget::Investment.feasible.compatible.valuation_finished.by_heading(heading_for_geozone(params[:geozone_id])) }
  ...
  private
    ...
    def stats_cache(key, &block)
      Rails.cache.fetch("spending_proposals_stats/20190206172207/#{key}", &block)
    end
